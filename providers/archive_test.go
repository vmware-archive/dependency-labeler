...
var _ = Describe("Providers/Archive", func() {
	Describe("ValidateURLs", func() {
		...
		It("returns an error if there is an invalid file extension", func() {
			err := providers.ValidateURLs([]string{"http://www.somewebsite.com/file_wrong.ext"}, func(u string) (*http.Response, error) {
				return &http.Response{StatusCode: 200}, nil
			})
		...
		DescribeTable("with a valid extension", func(validateUrl string) {
			err := providers.ValidateURLs([]string{validateUrl}, func(u string) (*http.Response, error) {
				return &http.Response{StatusCode: 200}, nil
			})
