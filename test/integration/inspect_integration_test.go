...
var _ = Describe("deplab inspect", func() {
	...
	Context("image is already deplab'd", func() {
		DescribeTable("prints all the metadata", func(flag, path string) {
			...
			Entry("[remote-image][private-registry] with a deplab'd image from a registry", "--image", "dependencylabeler/deplab-test-asset:tiny-deplabd"),
		)
	...
	DescribeTable("provides an error", func(flag, path, errorMsg string) {
		...
		Entry("[remote-image][private-registry] with a valid image from a registry with invalid json label", "--image", "dependencylabeler/deplab-test-asset:tiny-with-invalid-label", "cannot parse the label"),
	)
})
